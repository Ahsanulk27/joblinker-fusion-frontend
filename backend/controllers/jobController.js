...
exports.createJob = async (req, res) => {
    ...
    const { title, description, location, experience_required, status = 'open' } = req.body;
...
exports.addJobSkills = async (req, res) => {
    ...
    try {
      ...
      const [jobs] = await conn.query('SELECT * FROM jobs WHERE id = ? AND employer_id = ?', [jobId, userId]);
      if (jobs.length === 0) {
        conn.release();
        return res.status(403).json({ message: 'Unauthorized to modify this job or job not found' });
      }
...
exports.applyForJob = async (req, res) => {
    ...
    try {
      ...
      if (requiredSkills.length === 0) {
        return res.status(400).json({ message: 'Job has no defined required skills' });
      }
      ...
      const score = totalWeight > 0 ? ((matchedWeight / totalWeight) * 100).toFixed(2) : 0;
      ...
      conn.release();
      res.status(201).json({ message: 'Applied successfully', score: parseFloat(score) });
    } catch (err) {
      console.error(err);
